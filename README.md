...
## 🌿 Streamlit Web App (Upload an image → Get a prediction)

This repo includes a **very simple Streamlit app**:
- `app.py` (the web app)
- `class_names.json` (your label names)

### 1) Make sure your model is saved
Your training notebook uses:
- `MODEL_PATH = "../models/image_classification_model.keras"`

So your saved model should be at:
- `models/image_classification_model.keras`
...
### Troubleshooting
- If the app says **model not found** → train + save the model first.
- If the app predictions look wrong → check the order inside `class_names.json`.
